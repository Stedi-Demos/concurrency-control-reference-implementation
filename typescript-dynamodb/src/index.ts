...
class DynamoDBSemaphore {
  ...
  constructor(client: DynamoDBClient, config: SemaphoreConfig) {
    ...
    if (this.config.debug) {
      console.log(
        `DynamoDBSemaphore initialized with config:`,
    ...
    while (true) {
      try {
        ...
        if (retryCount >= maxRetries) {
          if (this.config.debug) {
            console.error(
              `Max retries (${maxRetries}) reached. Throwing error:`,
              error
            );
          }
          throw error;
        ...
        if (this.config.debug) {
          console.log(
            `Retry ${retryCount + 1}/${maxRetries}. Waiting for ${delay.toFixed(2)}ms before next attempt.`
          );
        }
  ...
  async acquireLock(): Promise<string | null> {
    if (this.config.debug) {
      console.log(
        `Attempting to acquire lock for semaphore: ${this.config.semaphoreName}`
      );
    }
    return this.retryWithBackoff(async () => {
      ...
      try {
        ...
        if (this.config.debug) {
          console.log(
            `Successfully incremented count for semaphore: ${this.config.semaphoreName}`
          );
        }
      ...
      if (!deleted) {
        try {
          ...
          if (error.name === "ConditionalCheckFailedException") {
            if (this.config.debug) {
              console.log(
                `Lock record not found, assuming already released: ${lockId}`
              );
            }
            return true;
          }
      ...
      try {
        await this.docClient.send(
          new UpdateCommand({
            ...
            Key: { pk: this.config.semaphoreName, sk: "COUNT" },
            UpdateExpression:
              "SET #count = if_not_exists(#count, :zero) - :dec",
        ...
        if (error.name === "ConditionalCheckFailedException") {
          ...
          if (this.config.debug) {
            console.log(
              `Condition check failed when decrementing count, assuming successful release for lock: ${lockId}`
            );
          }
        } else {
  ...
  async cleanupExpiredLocks(): Promise<void> {
    if (this.isCleaningUp) {
      if (this.config.debug) {
        console.log(
          `Cleanup already in progress for semaphore: ${this.config.semaphoreName}`
        );
      }
      return;
    }
    ...
    if (this.config.debug) {
      console.log(
        `Starting cleanup of expired locks for semaphore: ${this.config.semaphoreName}`
      );
    }
    ...
    try {
      await this.retryWithBackoff(async () => {
        ...
        try {
          ...
          const queryResult = await this.docClient.send(
            new QueryCommand({
              ...
              KeyConditionExpression:
                "pk = :pk AND begins_with(sk, :lockPrefix)",
              ExpressionAttributeValues: {
                ":pk": this.config.semaphoreName,
                ":lockPrefix": "LOCK#",
              },
          ...
          if (this.config.debug) {
            console.log(
              `Found ${allLocks.length} total locks, ${expiredLocks.length} expired`
            );
          }
          ...
          const currentCount =
            countResult.Items && countResult.Items[0]
          ...
          if (this.config.debug) {
            console.log(
              `Current count: ${currentCount}, Actual lock count: ${actualLockCount}`
            );
          }
 ...
 */
async function testAcquireMaxLocks(
  semaphore: DynamoDBSemaphore
): Promise<string[]> {
 ...
 */
async function testReleaseLocks(
  semaphore: DynamoDBSemaphore,
...
async function testThroughput(client: DynamoDBClient) {
  ...
  assert(
    ...
    "No successful operations in throughput test"
  );
